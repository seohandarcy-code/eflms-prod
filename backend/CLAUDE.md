...
- 모르는 컬럼(헤더가 `INPUT_COLUMNS`의 `name`/`label` 어디에도 없음)은 실패시키지 않고 `equipment_extra_attribute` 테이블에 원문 그대로 보관된다.
...
1. **컬럼 이름/한글 라벨 맞추기** — 실제 파일의 헤더가 `app/equipment_types/ef1_transformer.py`의 `INPUT_COLUMNS`에 있는 `name`(영문) 또는 `label`(한글) 중 하나와 정확히 일치해야 매칭된다. 실제 파일 헤더가 다르면 **`INPUT_COLUMNS`의 `label` 값을 실제 파일 헤더 표기에 맞게 고치는 것**이 제일 빠른 방법(새 컬럼 자체는 추가할 필요 없음, 이름/라벨 문자열만 조정). 안 맞는 컬럼은 실패하지 않고 `equipment_extra_attribute`로 빠지니, 처음엔 그냥 돌려보고 "미등록 컬럼" 목록으로 뭐가 안 맞는지 확인한 뒤 라벨을 맞춰가는 방식도 가능.
